...
Last updated: 2026-09-03 (M1 read stack landed on
`feat/gd-read-stack`, smoke-verified on the real install)
...
ARCHITECTURE.md "Crate layering"). The **read stack is done** on
`feat/gd-read-stack`: a four-crate workspace — `univault-engine`
(tq-univault's parsers vendored, with the GD LZ4 dialect),
`univault-io` (safe-io with post-write re-read), `univault-ui`
(art-free egui kit), `grimvault-core` (rolling-XOR codec,
`player.gdc` / `*.gst` parsers, layered game-data facade) — 155 tests,
clippy pedantic clean, and the `smoke` example resolves every item in
the user's three characters and the transfer stash to in-game names,
rarity, and footprints with byte-identical round-trips. Two binding
facts learned building it: **opaque save blocks cannot be re-keyed**,
so `player.gdc` stays read-only until blocks 5–17 are typed (yagde,
MIT, has read+write layouts for all of them) while `transfer.gst` is
writable now; and `formulas.gst` is plaintext TQ-style, not
obfuscated. Next: M2, the first usable loop — vault items out of the
transfer stash into `vault-store.json` and back — then M3, the egui
shell. grim-vault is the Grim Dawn sibling of tq-univault
(`~/Projects/tq-univault`); PROJECT.md is bound with `tracker: none`.
...
| `feat/gd-read-stack` | workspace + read stack (M1) | local, not pushed (no remote yet); 155 tests green |

## Next up

1. **M2 — first usable loop (core, pure):** `store.rs`
   (`vault-store.json`, format tag `grimvault-store`, monotonic
   `StoredItemId`, entries carry the full GD `Item` DTO), `transfer.rs`
   (move between transfer-stash tabs and the store; first-fit
   placement via engine `grid` with footprints from `gamedata`), a
   `TransferStash` edit + `encode` path gated by the baseline
   round-trip; an example that vaults an item from a **copy** of
   `transfer.gst` and puts it back, verified by re-parse. Writes go
   through `univault_io::backup_first_write` with
   `BackupPolicy::new("grimvault-bak", 5)`.
2. **M3 — egui shell (`crates/grimvault-gui`):** character picker
   (read-only inventory / equipped / personal stash), transfer-stash
   tabs (editable), store buckets by class; drag-and-drop; autosave
   (600 ms quiet) + external-change guard; game/save dir discovery;
   icons via `tex::decode` (DXT unsupported — fallback tile);
   grim-vault's own textures/fonts through `univault-ui`.
3. **Type `player.gdc` blocks 5–17** by porting yagde (MIT,
   `wr8fdy/yagde`, read+write for every block) so character
   inventories become writable under the lossless gate.
4. Deferred: extract the `univault-*` crates to their own repo and
   re-point tq-univault (R1–R4 already done on the vendored copies).
