...
//! [`FORMAT_TAG`]) under the app's config directory, shared by the
//! desktop app and the command-line examples so a path given once
//! serves every tool. Pure: parsing and serializing only; reading and
//! writing the file is the shell's job. The config directory is the
//! platform's unless [`CONFIG_DIR_ENV`] overrides it, which is how a
//! headless run avoids creating the real one.
...
impl Settings {
    ...
    #[expect(
        ...
        reason = "serializing two paths cannot fail; the expect states the invariant"
    )]
